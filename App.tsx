          ...
          {activeTab === 'logs' && (() => {
            const filteredLogs = (data.logs || []).filter(log => {
              ...
              return (
                ...
                new Date(log.timestamp).toLocaleString('id-ID').toLowerCase().includes(query)
              );
            });
            const totalLogsPages = Math.ceil(filteredLogs.length / logsItemsPerPage) || 1;
                    ...
                    <div className="flex items-center gap-2 bg-white dark:bg-slate-900 px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                      ...
                      <Dropdown 
                        ...
                        options={[
                          ...
                          { label: '50', value: 50 },
                          { label: '100', value: 100 },
                          { label: 'Semua', value: (data.logs || []).length || 1000 }
                        ]}
                        onChange={(val) => {
                          setLogsItemsPerPage(Number(val));
                          setLogsPage(1);
                        }}
                        className="w-24"
                      />
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors duration-300">
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      <thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                        <tr>
                          <th className="px-6 py-4 font-bold text-slate-600 dark:text-slate-400 text-sm uppercase tracking-wider whitespace-nowrap">Waktu</th>
                          <th className="px-6 py-4 font-bold text-slate-600 dark:text-slate-400 text-sm uppercase tracking-wider whitespace-nowrap">User</th>
                          <th className="px-6 py-4 font-bold text-slate-600 dark:text-slate-400 text-sm uppercase tracking-wider whitespace-nowrap">Aksi</th>
                          <th className="px-6 py-4 font-bold text-slate-600 dark:text-slate-400 text-sm uppercase tracking-wider whitespace-nowrap">Entitas</th>
                          <th className="px-6 py-4 font-bold text-slate-600 dark:text-slate-400 text-sm uppercase tracking-wider w-full">Detail</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {filteredLogs.slice().reverse().slice((logsPage - 1) * logsItemsPerPage, logsPage * logsItemsPerPage).map(log => (
                          <tr key={log.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                            ...
                            <td className="px-6 py-4 text-sm font-medium text-slate-700 dark:text-slate-200 whitespace-nowrap">{log.user}</td>
                            ...
                            <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-400 font-medium whitespace-nowrap">{log.entity}</td>
                            <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-400 min-w-[300px]">{log.details}</td>
                          </tr>
                        ))}
                        {filteredLogs.length === 0 && (
                          <tr>
                            <td colSpan={5} className="px-6 py-12 text-center text-slate-400 italic">
                              {logsSearchQuery ? 'Tidak ada riwayat yang cocok dengan pencarian' : 'Belum ada riwayat aktivitas'}
                            </td>
                          </tr>
                        )}
                      </tbody>
                  ...
                  <div className="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                    <span className="text-xs text-slate-500 dark:text-slate-400">
                      Halaman {logsPage} dari {totalLogsPages}
                    </span>
                    <div className="flex gap-2">
                      <button 
                        disabled={logsPage === 1}
                        onClick={() => setLogsPage(prev => Math.max(1, prev - 1))}
                        className="p-2 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 disabled:opacity-50 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
                      >
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
                      </button>
                      <button 
